...
{
  GC_CAN_IGNORE jit_insn *ref, *ref1, *ref2, *ref3, *chaperone_of_check, *ref_not_star;
  ...
  if (setter) {
    jit_subi_p(JIT_RUNSTACK, JIT_RUNSTACK, WORDS_TO_BYTES(5));
    CHECK_RUNSTACK_OVERFLOW();
  ...
  jit_stxi_p(WORDS_TO_BYTES(3), JIT_RUNSTACK, JIT_R0);
  jit_stxi_p(WORDS_TO_BYTES(2), JIT_RUNSTACK, JIT_R1);
  ...
  if (setter) jit_ldxi_p(JIT_R0, JIT_RUNSTACK, WORDS_TO_BYTES(5));
  else jit_ldxi_p(JIT_R0, JIT_RUNSTACK, WORDS_TO_BYTES(4));
  jit_str_p(JIT_RUNSTACK, JIT_R0);
  CHECK_LIMIT();
  JIT_UPDATE_THREAD_RSPTR();
  __END_SHORT_JUMPS__(1);
  ...
  chaperone_of_check = jit_jmpi(jit_forward()); /* jump over non-chaperone-vector* code */

  /* not a chaperone-vector*, stack is one too tall, fix then interpose */
  ...
  mz_patch_branch(chaperone_of_check);
